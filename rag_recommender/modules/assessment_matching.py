...
# Load configuration
CONFIG = get_config()
ASSESSMENT_CONFIG = CONFIG.get("assessment_matching", {})
...
def get_assessment_boosts(query: str, results_df) -> Dict[int, float]:
    ...
    for idx, row in results_df.iterrows():
        ...
        # 1. Check for category matches (using config value)
        for category, keywords in ASSESSMENT_CATEGORIES.items():
            if any(keyword.lower() in query_lower for keyword in keywords):
                ...
                if any(word.lower() in assessment_name.lower() for word in keywords):
                    boost_factor *= ASSESSMENT_CONFIG.get("category_match_boost", 2.8)
                
        # 2. Check for test type matches (using config value)
        if test_type != "nan":
            for type_name, type_keywords in TEST_TYPES.items():
                if type_name.lower() == test_type:
                    if any(keyword.lower() in query_lower for keyword in type_keywords):
                        boost_factor *= ASSESSMENT_CONFIG.get("test_type_match_boost", 1.85)
        
        # 3. Check for duration requirements with enhanced pattern matching (using config values)
        ...
        if duration_match and str(duration) != "nan":
            try:
                ...
                # More flexible scoring based on how close the duration is (using config values)
                if duration_diff <= 5:  # Very close match (within 5 minutes)
                    boost_factor *= ASSESSMENT_CONFIG.get("duration_close_match", 2.25)
                elif duration_diff <= 15:  # Close match (within 15 minutes)
                    boost_factor *= ASSESSMENT_CONFIG.get("duration_approximate_match", 1.75)
                elif duration_diff <= 30:  # Approximate match (within 30 minutes)
                    boost_factor *= ASSESSMENT_CONFIG.get("duration_range_match", 1.4)
            except (ValueError, TypeError):
                pass  # Skip if we can't convert the duration to a number
        
        # 4. Boost exact assessment name matches (using config value)
        for word in query_lower.split():
            if len(word) > 3 and word.lower() in assessment_name.lower():
                boost_factor *= ASSESSMENT_CONFIG.get("exact_name_match", 1.5)
        
        # 5. Remote testing preference - if specified in query (using config value)
        if "remote" in query_lower and remote == "yes":
            boost_factor *= ASSESSMENT_CONFIG.get("remote_preference", 1.25)
        
        # 6. Adaptive testing preference - if specified in query (using config value)
        if "adaptive" in query_lower and adaptive == "yes":
            boost_factor *= ASSESSMENT_CONFIG.get("adaptive_preference", 1.25)
            
        # Add small boost for assessments with complete information (using config value)
        if test_type != "nan" and str(duration) != "nan" and remote != "nan":
            boost_factor *= ASSESSMENT_CONFIG.get("complete_info_boost", 1.15)
